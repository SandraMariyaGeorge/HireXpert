...
class Jobs(Base):
    def scrape(self):
        ...
        for page in range(1, num_pages + 1):
            print(f"\n--- Scraping Page {page} ---")
            self.scrape_microsoft_jobs(base_api_url, page)
    ...
    def scrape_microsoft_jobs(self,api_url, page_number):
            ...
            if data["operationResult"]["status"] == "Success":
                ...
                for job in jobs:
                    ...
                    try:
                        ...
                        job_details['description'] = self.clean_text(result.get('description', ''))
                        job_details['qualification'] = self.clean_text(result.get('qualifications', ''))
                        job_details['responsibilities'] = self.clean_text(result.get('responsibilities', ''))

                        print(f"Processing job {job_details['job_id']}: {job_details['job_title']}")
                        self.create_embeddings(job_details)
